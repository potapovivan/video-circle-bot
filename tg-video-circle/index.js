...
if (!process.env.BOT_TOKEN) {
  console.error(
    "❌ BOT_TOKEN отсутствует. Добавь его в Railway Environment Variables."
  );
  process.exit(1);
}
...
bot.start((ctx) =>
  ctx.reply("👋 Пришли мне видео — сделаю из него кружок со звуком 🎥")
);
...
bot.on("video", async (ctx) => {
  ...
  try {
    ...
    // 3️⃣ Конвертируем в квадрат, максимум 60 сек, сохраняем звук
...
bot
  ...
  .catch((err) => {
    ...
    if (err.message.includes("409")) {
      console.log("⚠️ Найден другой экземпляр, перезапуск через 30 сек...");
...
http
  ...
  .listen(process.env.PORT || 10000, () => {
    console.log("🌐 Web server started for Railway keep-alive");
  });
